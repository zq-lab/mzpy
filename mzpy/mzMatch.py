...
import numpy as np
import pandas as pd
# 尝试导入 numba，如果失败，则定义一个空装饰器
try:
    from numba import njit
    has_numba = True
except ImportError:
    print('Recommend installing numba to accelerate the cosine algorithm.')
    def njit(*args, **kwargs):
        def decorator(func):
            return func  # 返回原始函数，不做任何修改
        return decorator
    has_numba = False
 
@njit if has_numba else lambda x: x 
def _cosine_(a, b, tol=0.005):
    """
    计算两条 msms 谱 a, b 的“匹配率修正”余弦相似度。
    a, b: shape (N,2), (M,2)，分别表示 [mz, intensity]，且均为 numpy.ndarray。
    tol: 峰匹配的公差 (绝对差值)
    ...
    """
    # 按 mz 升序排序
    sort_idx_a = np.argsort(a[:, 0])
    sort_idx_b = np.argsort(b[:, 0])
    ...
    # 准备并集数组(最大长度=n_a+n_b)
    union_a = np.empty(n_a + n_b, dtype=a.dtype)
    union_b = np.empty(n_a + n_b, dtype=b.dtype)
    ...
    # 计算余弦
    norm_a = np.sqrt(np.sum(union_a * union_a))
    norm_b = np.sqrt(np.sum(union_b * union_b))
    if norm_a == 0 or norm_b == 0:
        return 0.0

    dot_val = np.sum(union_a * union_b)
...
def warmup():
    """创建小数组进行热身以预编译 njit 函数"""  
    ref_ms = np.array([[100.0, 1.0]], dtype=np.float32)
    _cosine_(ref_ms, ref_ms)  # 直接调用 _cosine_ 函数进行热身
...
class Match:
    '''
    Performance
        Calculating internal similarity for 1,200 lines of data, took 0.002519s
    '''
    def __init__(self, ref_mz, ref_ms, que_mz=None, que_ms=None, sort_ms=False):
        """
        初始化 Match 类：
          1) 如果 que_mz, que_ms 为空，则认为自对比。
          2) 将数据转换为 float32 格式。
          3) 如果需要排序，则按 mz 排序。
        ...
        # 转换为 float32
        self.ref_mz = np.asarray(ref_mz, dtype=np.float32)
        self.ref_ms = []
        for item in ref_ms:
        ...
        if self.is_symmetric:
            ...
            self.que_mz = np.asarray(que_mz, dtype=np.float32)
            self.que_ms = []
            for item in que_ms:
                arr = np.asarray(item, dtype=np.float32)
                if sort_ms:
    ...
    def cosine_mx(self, tol=(0.003, 0.005)):
        """
        计算两批母离子 ref_mz, query_mz 的匹配后 msms 余弦相似度矩阵 (匹配率修正版本)。
        ...
        返回
        ----------
        sim_matrix : numpy.ndarray, shape (R, Q)
            sim_matrix[i, j] = 当 |ref_mz[i] - query_mz[j]| <= tol[0] 时，计算匹配率修正的余弦相似度；
                               否则为 0.0。
        """
        if self.is_symmetric:
            que_mz = self.ref_mz  # 这是临时赋值，不要去更改 self.que_mz and self.que_ms
            que_ms = self.ref_ms
        else:
            que_mz = self.que_mz
            que_ms = self.que_ms
        ...
        Q = que_mz.shape[0]
        similarity = np.full((R, Q), -1.0, dtype=np.float32)
        # 广播比对母离子
        diff_mz = np.abs(self.ref_mz[:, None] - que_mz[None, :])
        matched_mask = diff_mz <= tol[0]
        matched_i, matched_j = np.where(matched_mask)
        ...
        if self.is_symmetric:
            ...
            for k in range(matched_i.shape[0]):
                ...
                val = _cosine_(self.ref_ms[i], que_ms[j], tol[1])
                similarity[i, j] = val
    ...
    def cosine_df_chunk(cls, df, num_chunks, csv_fname, mz_on='precursormz', ms_on='msms', tol=(0.003, 0.005),
        ...
        超大数据表内部的MS相似度计算
        分块推进
        之所以按块推进，是为了充分使用内存，若按行逐行向下计算，由于来回转储，时间开销大
    ...
    def to_csv(self, score_mx, filename, mode='w'):
        ...
        if mode.lower() in ("a", 'append'):
            with open(filename, mode, newline='') as f:
                np.savetxt(f, np.column_stack(data), delimiter=',', fmt='%s,%s,%f')
        else:
            with open(filename, mode, newline='') as f:
